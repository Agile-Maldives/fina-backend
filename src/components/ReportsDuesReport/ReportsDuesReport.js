...
class ReportsDuesReport extends Component {
    componentDidMount() {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (this.props.selectedAccountId!== prevProps.selectedAccountId) {
            this.props.initDuesReport(this.props.selectedAccountId);
        }
    }
    ...
    render() {
        ...
        if(this.props.selectedAccountId !=null){
            downloadButtons = [
                <Button key="1"
                        href={"http://api.msresortsrentals.com/api/dues-per-loan-for-account-report-pdf/" +
                              this.props.selectedAccountId}
                >PDF</Button>,
                <Button key="2"
                        href={"http://api.msresortsrentals.com/api/dues-per-loan-for-account-report-csv/" +
                              this.props.selectedAccountId}
                >CSV</Button>,
            ]
        }
        ...
        return (
            <div>
                        ...
                        <PageHeader
                            ...
                            extra={[
                                ...
                                <Dropdown overlay={accountsMenu} placement="bottomLeft" trigger={['click']}>
                                    <Button type="primary">{captionAccountSelect}</Button>
                                </Dropdown>
                ...
                <Row gutter={16,16}>
                    <Col span={24}>
                        <Table
                            dataSource={this.props.duesReport}
                            columns={columns}
                        />
                    </Col>
                </Row>
...
const mapStateToProps = state => {
    return {
        ...
        duesReport:state.duesReport.duesReport,
        duesReportLoading: state.duesReport.duesReportLoading,
        duesReportMessage: state.duesReport.duesReportMessage,
        duesReportError: state.duesReport.duesReportError,
    }
}
const mapDispatchToProps = dispatch => {
    return {
        ...
        initDuesReport: (selectedAccountId) => dispatch(actions.initDuesReport(selectedAccountId)),
    }
}

export default connect(mapStateToProps, mapDispatchToProps)(ReportsDuesReport);
